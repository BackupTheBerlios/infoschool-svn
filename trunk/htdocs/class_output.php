 ...
 */
 ...
 class output {
  ...
  function output() {
   $this->root = $root = $GLOBALS['root'];
   $this->tmpl = new tmpl('output.html',array(),$root);
   $this->vars = array();
  }
  ...
  function secure($auth='user',$passwd=false) {
   ...
   if ($passwd && isset($_GET['passwd'])) {
    include $this->root.'../etc/random.php';
  ...
  function usermenu2vars_loggedin() {
   ...
   if($msg>0){
    $loggedin[]['usermenu'] = new tmpl('loggedin_msg.html',array('msg_num'=>$msg),$this->root);
   }
  ...
  function global2vars() {
   ...
   $loadtime = millitimestamp() - $GLOBALS['millitimestamp0'];
