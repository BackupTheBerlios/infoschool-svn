 ...
 */
 ...
 class entry {
  ...
  function load_answers($depth=1,$right='') {
   ...
   $query.=  'and (
               level<="'.$depth.'"
               or forum.created>"'.$_SESSION['last_login'].'"
              )';
   }
   $query.= 'order by
              forum_relation.level,
   ...
   while ($data = mysql_fetch_array($result)) {
    ...
    if (!isset($answer)) {
     ...
     if ($data['created'] > $_SESSION['last_login']) {
      $answer->new = true;
      $this->new_entries[$id] = &$answer;
      $level_diff = $data['level'] - $depth;
      if ($level_diff > 0) {
       $answer->load_history();
       for ($i=1;$i<$level_diff;$i++) {
        $parent_id = $answer->history[$i]->id;
        if (!isset($this->entries[$parent_id])) {
         $this->entries[$parent_id] = $answer->history[$i];
         $parent = &$answer->history[$i];
         $this->levels[$level-$i][$parent->id] = &$parent;
        }
       }
      }
     }
   ...
   for ($i=1;$i<=count($this->levels);$i++) {
    foreach ($this->levels[$i] as $id => $entry) {
     ...
     if ($answer->right_to($right)) {
      ...
      if (!$parent->new && $answer->new) {
       $this->new_answers[$answer->id] = &$answer;
      }
     }
     else {
      unset($this->new_entries[$answer->id]);
     }
    }
   }
  }
  ...
  function format($level=0,$levelinfo=array()) {
   $this->format_data();
