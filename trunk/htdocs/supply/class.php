 ...
 */
 ...
 class supply {
  ...
  function set_dir($dir='supply/') {
   $this->dir = $GLOBALS['root'].'../var/'.$dir;
  }

  function load_list() {
   if (!is_readable($this->dir)) return false;
   $dir = opendir($this->dir);
   if (!$dir) return false;
   $list = array();
   while ($entry = readdir($dir)) {
    if (!is_file($this->dir.$entry)) continue;
    if (!is_readable($this->dir.$entry)) continue;
    $name_ext = explode('.',$entry);
    if (!isset($name_ext[1]) || $name_ext[1] != 'txt') continue;
    $entry_timestamp = strtotime($name_ext[0]);
    $entry_date = date('Y-m-d',$entry_timestamp);
    $list[$entry_date] = $entry;
   }
   ksort($list);
   $this->list = $list;
  }
  ...
  function next_day() {
   $list = $this->list;
   $next_day = $day = $this->day;
   while ((list($date,$file) = each($list)) && $day >= $next_day) {
    $next_day = $this->daynum($date);
   }
   $this->set_day($next_day);
  }
  ...
  function get_day($day=false) {
   ...
   $this->set_day($day);
   if (!is_readable($this->path)) $this->next_day();
  }
  ...
  function set_day($day=0) {
   ...
   $this->date = $date;
   $this->path = $this->dir.$date.'.txt';
  }

  function format_menu() {
   $today = date('Y-m-d');
   $menu = array();
   foreach ($this->list as $date => $file) {
    if ($date < $today) continue;
    $entry[0] = array(
      'date' => tmpl_date_title($date,'md'),
      'day' => $this->daynum($date),
    );
    $link = $nolink = array();
    if ($date == $this->date) {
     $nolink = $entry;
  ...
  function import($fileid) {
   ...
   if (!is_writeable($this->dir)) die('ERROR: cannot write into directory');
   $this->path = $tmp_path = $this->dir.'tmp';
   move_uploaded_file($_FILES[$fileid]['tmp_name'],$tmp_path);
   $this->load_nameids();
   $this->load_data();
   $this->data2line();
   $this->extract_date();
   rename($tmp_path,$this->path);
   $this->analyse();
   $this->import_classes();
  ...
  function mail($verbose=0) {
   ...
   $this->load_data();
   if ($this->data == '') {
    $log->add('no supply schedule available');
    return false;
   }
   $data = html2text($this->data);
   global $db;
   ...
   $mail->data_main = html2text($this->data);
   $output = '';
   foreach ($db->data as $i => $person) {
    $name = html2text($person['name']);
